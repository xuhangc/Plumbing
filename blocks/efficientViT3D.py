...
"""
Conv3d_BN：该类定义了一系列3D卷积层，后跟批量归一化。它使用参数如卷积核大小、步长、填充等进行初始化。它还具有一个名为 fuse() 的方法，用于将卷积层与其批量归一化对应部分融合。

BN_Linear：与Conv3d_BN类似，但用于1D线性层。它将批量归一化与线性层融合。

Residual：为模型实现了残差连接。

FFN（前馈网络）：定义了一个简单的前馈网络，包含两个逐点卷积。

CascadedGroupAttention3D：该模块实现了级联组注意力机制。它计算3D数据的注意力图。

LocalWindowAttention3D：实现了局部窗口注意力，将输入划分为较小的窗口，并在这些窗口内执行注意力计算。

EfficientViTBlock3D：为高效的3D视觉变换器定义了一个块，其中包括深度可分离卷积、前馈网络和自注意机制。

PatchMerging3D 和 PatchExpand3D：这些模块处理了在3D空间中合并和扩展补丁。

这些组件是构建3D视觉变换器架构的关键构建块，能够处理时空数据，如视频或体积图像。
"""
...
if __name__ == '__main__':
    ...
    # 实例化EfficientViTBlock3D
    eff_vit_block = EfficientViTBlock3D(type='s', ed=channels, kd=8)

    # 打印输入形状
    print("Input shape:", input_data.shape)

    # 通过EfficientViTBlock3D传递数据
    output_data = eff_vit_block(input_data)
    print("EfficientViTBlock3D output shape:", output_data.shape)