...
"""
knn 函数：该函数根据欧氏距离计算点云中每个点的 k 个最近邻居。

get_graph_feature 函数：该函数通过将每个点与其邻居之间的特征差异以及该点本身的特征进行拼接，构建图特征。

GraphBlock 类：该类定义了一个图卷积块，用于处理点特征及其局部邻域特征，使用多尺度图卷积进行处理。
"""
...
if __name__ == '__main__':
    # 实例化 GraphBlock
    graph_block = GraphBlock(dim=64, k1=40, k2=20)

    # 定义输入张量，形状为 (batch_size, num_dims, num_points)
    batch_size = 1
    num_dims = 64
    num_points = 1024
    x = torch.randn(batch_size, num_dims, num_points).cuda()  # 假设使用 CUDA 进行计算

    # 将模型移动到 CUDA 设备
    graph_block = graph_block.cuda()

    # 进行前向传播
    output = graph_block(x)
