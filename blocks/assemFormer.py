...
实用函数和类
实用函数：
pair(Val)：如果 Val 是一个元组或列表，返回它；否则将其转化为一个包含 Val 的元组 (Val, Val)。
makeDivisible(v, divisor, min_value=None)：确保一个数能够被给定的除数整除。

自定义类：
Dropout：继承自 nn.Dropout，用于实现 dropout 机制。
StochasticDepth：继承自 torchvision.ops.StochasticDepth，用于实现随机深度机制。

自定义层和模块
BaseConv2d：自定义的卷积层，带有可选的批量归一化和激活函数。

LinearSelfAttention：
线性复杂度的自注意力层，适用于自注意力和交叉注意力。
qkv_proj：用于生成 query, key 和 value 的卷积层。
AttnDropRate：注意力分数的 dropout。
out_proj：输出投影的卷积层。

LinearAttnFFN：由自注意力和前馈网络组成的模块。
PreNormAttn：带有批量归一化和 dropout 的注意力层。
PreNormFfn：带有批量归一化和 dropout 的前馈网络层。

基础架构
BaseFormer：基础的 transformer 模块，结合了局部和全局表示。
LocalRep：局部表示模块，由一系列卷积层和 SE 层组成。
GlobalRep：全局表示模块，由多个注意力和前馈网络块组成。
unfolding 和 folding：用于将特征图展开为补丁，并将补丁重新折叠为特征图。

特定架构
AssemFormer：结合卷积和视觉 transformer 的模块。
ConvProj：用于将局部和全局特征组合的卷积层。
Dropout：随机深度机制。
"""
...
if __name__ == '__main__':
    ...
    model = AssemFormer(InChannels=64)
    output = model(input)
    print(output.shape)