...
class WTConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        self.wt_filter, self.iwt_filter = create_wavelet_filter_3d(wt_type, in_channels, torch.float)
        ...
        self.base_conv = nn.Conv3d(in_channels, in_channels, kernel_size, padding='same', stride=1, bias=bias)
        self.base_scale = _ScaleModule([1, in_channels, 1, 1, 1])

        self.wavelet_convs = nn.ModuleList(
            [nn.Conv3d(in_channels * 8, in_channels * 8, kernel_size, padding='same', stride=1, bias=False)
...
class DepthwiseSeparableConvWithWTConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3):
        super(DepthwiseSeparableConvWithWTConv3d, self).__init__()

        self.depthwise = WTConv3d(in_channels, in_channels, kernel_size=kernel_size)
...
if __name__ == '__main__':
    input_data = torch.randn(1, 64, 32, 32, 32)  # Example input data (batch_size, channels, depth, height, width)
    wtconv = DepthwiseSeparableConvWithWTConv3d(in_channels=64, out_channels=128)
