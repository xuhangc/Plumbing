...
#https://vllab.cs.nycu.edu.tw/images/paper/eccv_umam22.pdf
...
if __name__ == '__main__':
    # 定义注意力机制
    attention = Attention(dim_Q=1024, dim_K=1024, dim_LIN=1024, num_heads=8, n_pts=1024, ln=True)
    ...
    # 执行前向传播
    output, attention_scores = attention(Q, K, return_attn=True)

    print("输出形状:", output.shape)  # 应为 [BS, 1, 1024]
    print("注意力分数形状:", attention_scores.shape)  # 应为 [BS, 1024, 1]
