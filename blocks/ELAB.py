...
#Efficient Long-Range Attention Network for Image Super-resolution(ECCV 2022)
#https://arxiv.org/pdf/2203.06697
...
if __name__ == '__main__':
    # 实例化ELAB类
    # 假设输入通道数为3（例如RGB图像），输出通道数也设置为3（示例用途）
    elab = ELAB(inp_channels=3, out_channels=3)

    # 创建一个示例输入张量，这里假设我们有一个批量大小为1的3通道图像，尺寸为64x64
    input_tensor = torch.randn(1, 3, 1280, 720)

    # 将输入张量传递给ELAB模型
    output_tensor = elab(input_tensor)
