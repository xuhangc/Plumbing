...
if __name__ == '__main__':
    block = Xception(8).cuda()  # 假设您想保持downsample_factor为8
    input = torch.rand(1, 3, 512, 512).cuda()  # 输入图片大小为512*512，通道数为3（RGB图像）
    output = block(input)
    print(input.size(), output.size())  # 输出第一项为中间层特征图尺寸，第二项为最终输出尺寸