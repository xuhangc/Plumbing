...
#代码出处：https://zhuanlan.zhihu.com/p/208519425
#论文题目：《Dynamic Convolution: Attention over Convolution Kernels》
"""
注意力机制 (attention1d, attention2d, attention3d)

每个注意力模块根据输入张量计算注意力分数。它使用两个卷积层，然后是 softmax 函数来计算注意力分数。这些注意力分数确定输入张量不同部分的重要性。
温度参数控制注意力分布的“软度”。
动态卷积层 (Dynamic_conv1d, Dynamic_conv2d, Dynamic_conv3d)

这些层实现了动态卷积操作，根据注意力分数自适应地组合多个卷积滤波器。
它们首先使用相应的注意力机制计算注意力分数。
然后，根据注意力分数组合多组卷积滤波器，产生最终输出。
初始化

卷积层的权重使用 Kaiming 初始化，而偏置初始化为零。
更新温度

update_temperature 方法减少注意力机制的温度参数。此方法可能用于训练过程中的退火。
前向传递

每个模块的前向传递接受输入张量 x，计算注意力分数，应用动态卷积，然后返回输出张量。
"""
